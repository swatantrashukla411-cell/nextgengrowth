...
async function run() {
  console.log('🚀 Launching headful Chromium browser...');
  ...
  try {
    ...
    console.error('❌ Failed to launch browser. If a browser window is already open from this script, please close it first.', err);
    return;
  }
  ...
  // Wait up to 5 minutes for the user to log in if not already
  for (let i = 0; i < 150; i++) {
    ...
    if (i % 5 === 0) {
      console.log('ℹ️ Waiting for you to log in to Instagram in the opened browser window...');
    }
    await page.waitForTimeout(2000);
  }

  if (!loggedIn) {
    console.error('❌ Login timeout or not logged in. Exiting.');
    await context.close();
    return;
  }
  ...
  for (let idx = 0; idx < brands.length; idx++) {
    ...
    // Delay between messages to mimic human behavior and avoid rate limits
