...
// HTML template with personal plain-text appearance
function generateHREmailTemplate(name, company, pitch) {
  // Format pitch to start with a lowercase letter if it exists
...
async function main() {
  let csvPath = path.resolve(__dirname, 'prospects_database.csv');
  if (!fs.existsSync(csvPath)) {
    csvPath = path.resolve(__dirname, 'hr_contacts.csv');
  }

  if (!fs.existsSync(csvPath)) {
    console.error(`❌ CSV File not found. Make sure prospects_database.csv or hr_contacts.csv exists.`);
    process.exit(1);
  }
  ...
  try {
    ...
    if (lines.length <= 1) {
      console.log('⚠️ No contacts found in CSV file (only headers present).');
      return;
    }
    ...
    if (emailIndex === -1 || nameIndex === -1) {
      console.error('❌ Could not find "Name" or "Email" columns in the CSV file.');
    ...
    console.log(`📋 Loaded ${hrLeaders.length} HR leaders from CSV.`);
    console.log(`🚀 Starting Customer Discovery campaign via: ${gmailUser}...`);
    console.log('------------------------------------------------------------');
    ...
    for (let i = 0; i < hrLeaders.length; i++) {
      ...
      // 5-second rate-limiting delay between sends to ensure high inbox rate
    ...
    console.log('------------------------------------------------------------');
    console.log(`📊 HR Campaign Summary:`);
    ...
    const reportPath = path.resolve(__dirname, 'hr_campaign_report.json');
    fs.writeFileSync(reportPath, JSON.stringify(report, null, 2), 'utf8');
    console.log(`📁 Detailed log written to: ${reportPath}`);

  } catch (err) {
    console.error('❌ Critical error during HR campaign execution:', err.message);
  }
}
